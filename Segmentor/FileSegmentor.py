...
class FileSegmentor(DocSegmentor):
	def __init__(self, model_dir=__default_model_dir__, segment=True, postag=False, encoding='UTF-8',errors='strict'):
	...
	def procFile(self, infile, outfile=None):
		...
		if infile=="-":
			...
			inf=codecs.open(infile,"r",encoding=self.encoding, errors=self.errors)
		else:
			inf=infile
		...
		if not outfile:
			...
			outf=codecs.open(outfile,"w",encoding=self.encoding,errors=self.errors)
		elif outfile=="-":
			outf=sys.stdout
		elif not isinstance(outfile,IOBase):
			outf=codecs.open(outfile,"w",encoding=self.encoding)
		else:
			outf=outfile
